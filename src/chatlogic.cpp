...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
    ...
    ChatBot chatbot0("../images/chatbot.png");
    SetChatbotHandle(&chatbot0);
    // add pointer to chatlogic so that chatbot answers can be passed on to the GUI
    chatbot0.SetChatLogicHandle(this);
    chatbot0.SetRootNode(rootNode);
    rootNode->MoveChatbotHere(std::move(chatbot0));
}
